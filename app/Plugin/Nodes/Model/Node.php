 ...
 */
	public function saveNode($data, $typeAlias = self::DEFAULT_TYPE) {
		...
		Dna::dispatchEvent('Model.Node.beforeSaveNode', $this, compact('data'));
		$result = $this->saveWithMeta($data);
		Dna::dispatchEvent('Model.Node.afterSaveNode', $this, compact('data'));
 ...
 */
	public function processAction($action, $ids) {
		...
		if (!in_array($actionToPerform, array_keys($this->actionsMapping))) {
			throw new InvalidArgumentException(__d('dna', 'Invalid action to perform'));
		}

		if (empty($ids)) {
			throw new InvalidArgumentException(__d('dna', 'No target to process action upon'));
		}
 ...
 */
	public function formatData($data, $typeAlias = self::DEFAULT_TYPE) {
		...
		if (empty($type)) {
			throw new InvalidArgumentException(__d('dna', 'Invalid Content Type'));
		}
 ...
 */
	protected function _getNodeRelativePath($node) {
		return Dna::getRelativePath(array(
			'plugin' => 'nodes',
			'admin' => false,
			'controller' => 'nodes',
			'action' => 'view',
