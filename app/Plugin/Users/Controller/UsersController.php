 ...
 */
	public function onBeforeAdminLogin() {
		...
		if (Cache::read($cacheName, 'users_login') >= Configure::read('User.failed_login_limit')) {
			$this->Session->setFlash(__d('dna', 'You have reached maximum limit for failed login attempts. Please try again after a few minutes.'), 'default', array('class' => 'error'));
 ...
 */
	public function admin_index() {
		$this->set('title_for_layout', __d('dna', 'Users'));
	...
	public function admin_add() {
		if (!empty($this->request->data)) {
			...
			if ($this->User->save($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'The User has been saved'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__d('dna', 'The User could not be saved. Please, try again.'), 'default', array('class' => 'error'));
 ...
 */
	public function admin_reset_password($id = null) {
		if (!$id && empty($this->request->data)) {
			$this->Session->setFlash(__d('dna', 'Invalid User'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->request->data)) {
			if ($this->User->save($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'Password has been reset.'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__d('dna', 'Password could not be reset. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
		if (empty($this->request->data)) {
			$this->request->data = $this->User->read(null, $id);
 ...
 */
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('dna', 'Invalid id for User'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if ($this->User->delete($id)) {
			$this->Session->setFlash(__d('dna', 'User deleted'), 'default', array('class' => 'success'));
			$this->redirect(array('action' => 'index'));
		} else {
			$this->Session->setFlash(__d('dna', 'User cannot be deleted'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
	}
 ...
 */
	public function admin_login() {
		$this->set('title_for_layout', __d('dna', 'Admin Login'));
		$this->layout = "admin_login";
		if ($this->request->is('post')) {
			Dna::dispatchEvent('Controller.Users.beforeAdminLogin', $this);
			if ($this->Auth->login()) {
				Dna::dispatchEvent('Controller.Users.adminLoginSuccessful', $this);
				$this->redirect($this->Auth->redirect());
			} else {
				Dna::dispatchEvent('Controller.Users.adminLoginFailure', $this);
 ...
 */
	public function admin_logout() {
		Dna::dispatchEvent('Controller.Users.adminLogoutSuccessful', $this);
		$this->Session->setFlash(__d('dna', 'Log out successful.'), 'default', array('class' => 'success'));
 ...
 */
	public function add() {
		$this->set('title_for_layout', __d('dna', 'Register'));
		if (!empty($this->request->data)) {
			$this->User->create();
			...
			if ($this->User->save($this->request->data)) {
				Dna::dispatchEvent('Controller.Users.registrationSuccessful', $this);
				$this->request->data['User']['password'] = null;

				$this->_sendEmail(
					array(Configure::read('Site.title'), $this->_getSenderEmail()),
					$this->request->data['User']['email'],
					__d('dna', '[%s] Please activate your account', Configure::read('Site.title')),
					'Users.register',
					'user activation',
					$this->theme,
				...
				$this->Session->setFlash(__d('dna', 'You have successfully registered an account. An email has been sent with further instructions.'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'login'));
			} else {
				Dna::dispatchEvent('Controller.Users.registrationFailure', $this);
				$this->Session->setFlash(__d('dna', 'The User could not be saved. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
	}
 ...
 */
	public function activate($username = null, $key = null) {
		...
		if ($this->User->hasAny(array(
			...
			Dna::dispatchEvent('Controller.Users.activationSuccessful', $this);
			$this->Session->setFlash(__d('dna', 'Account activated successfully.'), 'default', array('class' => 'success'));
		} else {
			Dna::dispatchEvent('Controller.Users.activationFailure', $this);
 ...
 */
	public function forgot() {
		$this->set('title_for_layout', __d('dna', 'Forgot Password'));

		if (!empty($this->request->data) && isset($this->request->data['User']['username'])) {
			$user = $this->User->findByUsername($this->request->data['User']['username']);
			if (!isset($user['User']['id'])) {
				$this->Session->setFlash(__d('dna', 'Invalid username.'), 'default', array('class' => 'error'));
				$this->redirect(array('action' => 'login'));
			}
			...
			$emailSent = $this->_sendEmail(
				array(Configure::read('Site.title'), $this->_getSenderEmail()),
				$user['User']['email'],
				__d('dna', '[%s] Reset Password', Configure::read('Site.title')),
			...
			if ($emailSent) {
				$this->Session->setFlash(__d('dna', 'An email has been sent with instructions for resetting your password.'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'login'));
			} else {
				$this->Session->setFlash(__d('dna', 'An error occurred. Please try again.'), 'default', array('class' => 'error'));
			}
		}
	}
 ...
 */
	public function reset($username = null, $key = null) {
		$this->set('title_for_layout', __d('dna', 'Reset Password'));
		...
		if (!empty($this->request->data) && isset($this->request->data['User']['password'])) {
			...
			if ($this->User->save($user['User'], $options)) {
				$this->Session->setFlash(__d('dna', 'Your password has been reset successfully.'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'login'));
			} else {
				$this->Session->setFlash(__d('dna', 'An error occurred. Please try again.'), 'default', array('class' => 'error'));
			}
 ...
 */
	public function login() {
		$this->set('title_for_layout', __d('dna', 'Log in'));
		if ($this->request->is('post')) {
			Dna::dispatchEvent('Controller.Users.beforeLogin', $this);
			if ($this->Auth->login()) {
				Dna::dispatchEvent('Controller.Users.loginSuccessful', $this);
				$this->redirect($this->Auth->redirect());
			} else {
				Dna::dispatchEvent('Controller.Users.loginFailure', $this);
				$this->Session->setFlash($this->Auth->authError, 'default', array('class' => 'error'), 'auth');
				$this->redirect($this->Auth->loginAction);
			}
		}
 ...
 */
	public function logout() {
		Dna::dispatchEvent('Controller.Users.beforeLogout', $this);
		$this->Session->setFlash(__d('dna', 'Log out successful.'), 'default', array('class' => 'success'));
		$this->redirect($this->Auth->logout());
		Dna::dispatchEvent('Controller.Users.afterLogout', $this);
	}
 ...
 */
	...
	protected function _getSenderEmail() {
		return 'dna@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));
	}
