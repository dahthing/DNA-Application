...
echo $this->Dna->adminAction(
        __d('dna', 'Upload'), array('action' => 'add')
);
?>
<?php $this->end(); ?>
...
<table class="table table-striped">
    ...
    foreach ($plugins as $pluginAlias => $pluginData):
        ...
        if (!in_array($pluginAlias, $bundledPlugins)):
            $icon = $pluginData['active'] ? 'glyphicons no-js ban' : 'glyphicons no-js electricity';
            ...
            $actions[] = $this->Dna->adminRowAction('', array('action' => 'delete', $pluginAlias), 
                    array(
                        'class' => 'glyphicons bin', 
                        'tooltip' => __d('dna', 'Delete')
                    ), __d('dna', 'Are you sure?')
            );
        endif;

        if ($pluginData['active'] && !in_array($pluginAlias, $bundledPlugins)) {
            $actions[] = $this->Dna->adminRowAction('', array('action' => 'moveup', $pluginAlias), 
                    array(
                        'class' => 'glyphicons up_arrow', 
                        'tooltip' => __d('dna', 'Move up'), 
                        'method' => 'post'
                    ), __d('dna', 'Are you sure?')
            );

            $actions[] = $this->Dna->adminRowAction('', array('action' => 'movedown', $pluginAlias), 
                    array(
                        'class' => 'glyphicons down_arrow', 
