...
$this->Html
	...
	->addCrumb(__d('dna', 'Themes'), '/' . $this->request->url);
	...
	<div class="span12 extensions-themes">
		...
		<div class="current-theme row-fluid">
			<div class="screenshot span4">
				<h3><?php echo __d('dna', 'Current Theme'); ?></h3>
				<?php
			...
			<div class="span8">
				...
				<?php
					...
					if (!empty($author)):
						echo ' ' . __d('dna', 'by') . ' ' . $author;
					endif;
				?>
				</h3>
				<p class="description"><?php echo $currentTheme['description']; ?></p>
				<?php if (isset($currentTheme['regions'])): ?>
				<p class="regions"><?php echo __d('dna', 'Regions supported: ') . implode(', ', $currentTheme['regions']); ?></p>
				<?php endif; ?>
			</div>
		</div>

		<div class="available-themes row-fluid">
			<h3><?php echo __d('dna', 'Available Themes'); ?></h3>
			<ul>
			<?php
				$hasAvailable = false;
				...
				foreach ($themesData as $themeAlias => $theme):
					...
					$out = $this->Html->tag('h3', $theme['name'] . ' ' . __d('dna', 'by') . ' ' . $author, array());
					$out .= $this->Html->tag('p', $theme['description'], array('class' => 'description'));
					if (isset($theme['regions'])):
						$out .= $this->Html->tag('p', __d('dna', 'Regions supported: ') . implode(', ', $theme['regions']), array('class' => 'regions'));
					endif;
					$out .= $this->Html->tag('div',
						$this->Form->postLink(__d('dna', 'Activate'), array(
							'action' => 'activate',
							$themeAlias,
						), array(
							'button' => 'default',
						...
						$this->Form->postLink(__d('dna', 'Delete'), array(
							'action' => 'delete',
							$themeAlias,
						), array(
							'button' => 'danger',
