 ...
 */
	public function admin_index() {
		$this->set('title_for_layout', __d('dna', 'Locales'));
 ...
 */
	public function admin_activate($locale = null) {
		if ($locale == null || !is_dir(APP . 'Locale' . DS . $locale)) {
			$this->Session->setFlash(__d('dna', 'Locale does not exist.'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}

		$result = $this->Setting->write('Site.locale', $locale);
		if ($result) {
			$this->Session->setFlash(sprintf(__d('dna', "Locale '%s' set as default"), $locale), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'Could not save Locale setting.'), 'default', array('class' => 'error'));
		}
		$this->redirect(array('action' => 'index'));
	}
 ...
 */
	public function admin_add() {
		$this->set('title_for_layout', __d('dna', 'Upload a new locale'));
		...
		if ($this->request->is('post') && !empty($this->request->data)) {
			...
			if (is_dir(APP . 'Locale' . DS . $locale)) {
				$this->Session->setFlash(__d('dna', 'Locale already exists.'), 'default', array('class' => 'error'));
				$this->redirect(array('action' => 'add'));
			}
 ...
 */
	public function admin_edit($locale = null) {
		$this->set('title_for_layout', sprintf(__d('dna', 'Edit locale: %s'), $locale));
		...
		$poFile = APP . 'Locale' . DS . $locale . DS . 'LC_MESSAGES' . DS . 'dna.po';

		if (!file_exists($poFile)) {
			$this->Session->setFlash(__d('dna', 'The file %s does not exist.', basename($poFile)), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		...
		if (!empty($this->request->data)) {
			...
			if ($file->write($this->request->data['Locale']['content'])) {
				$this->Session->setFlash(__d('dna', 'Locale updated successfully'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'index'));
			}
		}
 ...
 */
	public function admin_delete($locale = null) {
		...
		if ($folder->delete(APP . 'Locale' . DS . $locale)) {
			$this->Session->setFlash(__d('dna', 'Locale deleted successfully.'), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'Local could not be deleted.'), 'default', array('class' => 'error'));
		}
