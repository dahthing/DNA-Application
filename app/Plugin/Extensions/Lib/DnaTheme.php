 ...
 */
	public function getData($alias = null) {
		if ($alias == null || $alias == 'default') {
			$manifestFile = CakePlugin::path('Dna') . 'webroot' . DS . 'theme.json';
		} else {
			$viewPaths = App::path('views');
			foreach ($viewPaths as $viewPath) {
 ...
 */
	public function delete($alias) {
		if (empty($alias)) {
			throw new InvalidArgumentException(__d('dna', 'Invalid theme'));
		}
		$paths = array(
		...
		throw new UnexpectedValueException(__d('dna', 'Theme %s not found', $alias));
	}
