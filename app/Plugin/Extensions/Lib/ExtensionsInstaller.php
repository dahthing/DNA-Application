...
/**
 * Hold instance of DnaComposer
 *
 * @var DnaComposer
 */
	protected $_DnaComposer = null;

/**
 * __construct
 */
	public function __construct() {
		$this->_DnaComposer = new DnaComposer();
	}
 ...
 */
	public function getPluginName($path = null) {
		if (empty($path)) {
			throw new CakeException(__d('dna', 'Invalid plugin path'));
		}
		...
		if ($Zip->open($path) === true) {
			...
			if ($indexJson === false) {
				throw new CakeException(__d('dna', 'Invalid zip archive'));
			} else {
 ...
 */
	public function extractPlugin($path = null, $plugin = null) {
		if (!file_exists($path)) {
			throw new CakeException(__d('dna', 'Invalid plugin file path'));
		}
		...
		if (is_dir($pluginPath)) {
			throw new CakeException(__d('dna', 'Plugin already exists'));
		}
		...
		if ($Zip->open($path) === true) {
			...
			throw new CakeException(__d('dna', 'Failed to extract plugin'));
		}
		return false;
	}
 ...
 */
	public function getThemeName($path = null) {
		if (empty($path)) {
			throw new CakeException(__d('dna', 'Invalid theme path'));
		}
		...
		if ($Zip->open($path) === true) {
			...
			$Zip->close();
			if (!$theme) {
				throw new CakeException(__d('dna', 'Invalid theme'));
			}
 ...
 */
	public function extractTheme($path = null, $theme = null) {
		if (!file_exists($path)) {
			throw new CakeException(__d('dna', 'Invalid theme file path'));
		}
		...
		if (is_dir($themePath)) {
			throw new CakeException(__d('dna', 'Theme already exists'));
		}
		...
		if ($Zip->open($path) === true) {
			...
			throw new CakeException(__d('dna', 'Failed to extract theme'));
		}
		return false;
	}

/**
 * Install packages with DnaComposer
 *
 * @param array $data
 * @return boolean
 * @throws CakeException
 */
	public function composerInstall($data = array()) {
		...
		if (empty($data['package']) || strpos($data['package'], '/') === false) {
			throw new CakeException(__d('dna', 'Must specify a valid package name: vendor/name.'));
		}
		if ($data['type'] == 'theme') {
			throw new CakeException(__d('dna', 'Themes installed via composer are not yet supported.'));
		}
		$this->_DnaComposer->getComposer();
		$this->_DnaComposer->setConfig(array(
			$data['package'] => $data['version'],
		));
		return $this->_DnaComposer->runComposer();
	}
