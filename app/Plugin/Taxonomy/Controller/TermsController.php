 ...
 */
	public function admin_index($vocabularyId = null) {
		...
		if (!isset($vocabulary['Vocabulary']['id'])) {
			$this->Session->setFlash(__d('dna', 'Invalid Vocabulary ID.'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'controller' => 'vocabularies',
				'action' => 'index',
			));
		}
		$this->set('title_for_layout', __d('dna', 'Vocabulary: %s', $vocabulary['Vocabulary']['title']));
 ...
 */
	public function admin_add($vocabularyId = null) {
		...
		$this->set('title_for_layout', __d('dna', '%s: Add Term', $vocabulary['Vocabulary']['title']));
		...
		if (!empty($this->request->data)) {
			...
			if ($termId) {
				...
				if ($termInVocabulary) {
					$this->Session->setFlash(__d('dna', 'Term with same slug already exists in the vocabulary.'), 'default', array('class' => 'error'));
				} else {
					...
					if ($this->Term->Taxonomy->save($taxonomy)) {
						$this->Session->setFlash(__d('dna', 'Term saved successfuly.'), 'default', array('class' => 'success'));
						$this->redirect(array(
							'action' => 'index',
							$vocabularyId,
						));
					} else {
						$this->Session->setFlash(__d('dna', 'Term could not be added to the vocabulary. Please try again.'), 'default', array('class' => 'error'));
					}
				}
			} else {
				$this->Session->setFlash(__d('dna', 'Term could not be saved. Please try again.'), 'default', array('class' => 'error'));
			}
		}
 ...
 */
	public function admin_edit($id = null, $vocabularyId = null) {
		...
		$this->set('title_for_layout', __d('dna', '%s: Edit Term', $vocabulary['Vocabulary']['title']));
 ...
 */
	public function admin_delete($id = null, $vocabularyId = null) {
		if (!$id || !$vocabularyId) {
			$this->Session->setFlash(__d('dna', 'Invalid id for Term'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'action' => 'index',
				$vocabularyId,
			));
		...
		if ($this->Term->Taxonomy->delete($taxonomyId)) {
			$this->Session->setFlash(__d('dna', 'Term deleted'), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'Term could not be deleted. Please, try again.'), 'default', array('class' => 'error'));
		}
		$this->redirect(array(
			'action' => 'index',
			$vocabularyId,
