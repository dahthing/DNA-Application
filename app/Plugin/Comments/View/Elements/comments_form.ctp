<div class="comment-form">
	<h3><?php echo __d('dna', 'Add new comment'); ?></h3>
	<?php
		$type = $types_for_layout[$node['Node']['type']];

		if ($this->params['controller'] == 'comments') {
			$nodeLink = $this->Html->link(__d('dna', 'Go back to original post') . ': ' . $node['Node']['title'], $node['Node']['url']);
		...
		echo $this->Form->create('Comment', array('url' => $formUrl));
			if ($this->Session->check('Auth.User.id')) {
				...
				echo $this->Form->input('Comment.name', array('label' => __d('dna', 'Name')));
				echo $this->Form->input('Comment.email', array('label' => __d('dna', 'Email')));
				echo $this->Form->input('Comment.website', array('label' => __d('dna', 'Website')));
				echo $this->Form->input('Comment.body', array('label' => false));
			}
		...
		echo $this->Form->end(__d('dna', 'Post comment'));
	?>
</div>