<div id="comment-<?php echo $comment['Comment']['id']; ?>" class="comment<?php if ($node['Node']['user_id'] == $comment['Comment']['user_id']) { echo ' author'; } ?>">
	<div class="comment-info">
		...
		<span class="date"><?php echo __d('dna', 'said on %s', $this->Time->format(Configure::read('Comment.date_time_format'), $comment['Comment']['created'], null, Configure::read('Site.timezone'))); ?></span>
	</div>
	...
	<?php
		if ($level <= Configure::read('Comment.level')) {
			echo $this->Html->link(__d('dna', 'Reply'), array(
				'plugin' => 'comments',
				'controller' => 'comments',
				'action' => 'add',
				$node['Node']['id'],
