...
App::uses('DnaControllerTestCase', 'Dna.TestSuite');
App::uses('DnaTestFixture', 'Dna.TestSuite');

class CommentsControllerTest extends DnaControllerTestCase {
 ...
 */
	public function testAdminEdit() {
		...
		$this->assertEquals('Mr Dna [modified]', $result['Comment']['name']);
	}
 ...
 */
	public function testAdminProcessDelete() {
		...
		$this->assertEqual($list, array(2 => 'Mrs Dna'));
	}
 ...
 */
	public function testAdd() {
		...
		$this->assertEqual($commenters, array('Mr Dna', 'Mrs Dna', 'John Smith'));
 ...
 */
	public function testAddWithParent() {
		...
		$Comments->add($node['Node']['id'], 1); // under the comment by Mr Dna
		$this->assertEqual($Comments->viewVars['success'], 1);

		$comments = $Comments->Comment->generateTreeList(array('Comment.node_id' => $node['Node']['id']), '{n}.Comment.id', '{n}.Comment.name');
		$commenters = array_values($comments);
		$this->assertEqual($commenters, array('Mr Dna', '_John Smith', 'Mrs Dna'));
