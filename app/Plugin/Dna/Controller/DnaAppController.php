 ...
 * PHP version 5
 *
 * @category Dna.Controller
 * @package  Dna.Dna.Controller
 * @version  1.5
 * @author   Fahad Ibnay Heylaal <contact@fahad19.com>
 * @license  http://www.opensource.org/licenses/mit-license.php The MIT License
 * @link     http://www.dna.org
 */
class DnaAppController extends Controller {
 ...
 */
	public function afterConstruct() {
		Dna::applyHookProperties('Hook.controller_properties', $this);
		if (isset($this->request->params['admin'])) {
			$this->helpers[] = 'Dna.Dna';
			if (empty($this->helpers['Html'])) {
				$this->helpers['Html'] = array('className' => 'Dna.DnaHtml');
			}
			if (empty($this->helpers['Form'])) {
				$this->helpers['Form'] = array('className' => 'Dna.DnaForm');
			}
			if (empty($this->helpers['Paginator'])) {
				$this->helpers['Paginator'] = array('className' => 'Dna.DnaPaginator');
			}
		}
	}
 ...
 */
	public function beforeFilter() {
		...
		if (!isset($this->request->params['admin']) &&
			...
			$this->set('title_for_layout', __d('dna', 'Site down for maintenance'));
...
/**
 * Dna uses this callback to load Paginator helper when one is not supplied.
