 ...
 * Use it only for DnaRouter::connect()
 *
 * @package  Dna.Dna.Lib
 * @version  1.0
...
class DnaRouter {
    ...
    public static function contentType($alias) {
        DnaRouter::connect('/' . $alias, array(
            'plugin' => 'nodes', 'controller' => 'nodes',
            'action' => 'index', 'type' => $alias
        ));
        DnaRouter::connect('/' . $alias . '/archives/*', array(
            'plugin' => 'nodes', 'controller' => 'nodes',
            'action' => 'index', 'type' => $alias
        ));
        DnaRouter::connect('/' . $alias . '/:slug', array(
            'plugin' => 'nodes', 'controller' => 'nodes',
            'action' => 'view', 'type' => $alias
        ));
        DnaRouter::connect('/' . $alias . '/term/:slug/*', array(
            'plugin' => 'nodes', 'controller' => 'nodes',
        ...
        try {
            ...
            foreach ($types as $type) {
                if (isset($type['Params']['routes']) && $type['Params']['routes']) {
                    DnaRouter::contentType($type['Type']['alias']);
                }
            }
        } catch (MissingConnectionException $e) {
            CakeLog::write('critical', __d('dna', 'Unable to get routeable content types: %s', $e->getMessage()));
        }
    }
