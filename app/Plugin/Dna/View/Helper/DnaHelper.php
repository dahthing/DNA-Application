 ...
 */
class DnaHelper extends AppHelper {

    public $helpers = array(
        'Form' => array('className' => 'Dna.DnaForm'),
        'Html' => array('className' => 'Dna.DnaHtml'),
        'Dna.Layout',
        'Menus.Menus',
    );
    ...
    /** Generate Admin menus added by DnaNav::add()
     *
     * @param array $menus
     * @param array $options
    ...
    public function adminMenus($menus, $options = array(), $depth = 0) {
        ...
        if (empty($this->Role)) {
            ...
            $this->Role->Behaviors->attach('Dna.Aliasable');
        }
     ...
     * @return string menu html tags
     */
    public function adminDnaMenus($menus, $options = array(), $depth = 0) {
        $options = Hash::merge(array(
                    'children' => true,
                    'htmlAttributes' => array(),
            ), $options);
        ...
        foreach ($sorted as $menu) {
            ...
            $title = '';
            if (empty($menu['icon'])) {
                //$menu['htmlAttributes']['class'] .= array('icon' => 'white');
            } else {
                $menu['htmlAttributes']['class'] .= ' '.$menu['icon'].' ';
            }
            ...
            if (!empty($menu['children'])) {
                ...
                $children = $this->adminDnaMenus($menu['children'], array(
                    'children' => true,
                    'htmlAttributes' => array('class' => $childClass),
                        ), $depth + 1);
            ...
            $link = $this->Html->link($this->Html->Tag('i','') . $title, $menu['url'], $menu['htmlAttributes']);
           
            $liOptions = array('class'=>'');
            $liOptions['class'] = $Active;
            if (!empty($children)) {
                $liOptions['class'] = ' hasSubmenu '.$Active;
            }
    ...
    public function adminRowAction($title, $url = null, $options = array(), $confirmMessage = false) {
        ...
        if ($action == 'delete' || isset($usePost)) {
            return $this->Form->postLink($this->Html->Tag('i','').$title, $url, $options, $confirmMessage);
        }

        return $this->Html->link($this->Html->Tag('i','').$title, $url, $options, $confirmMessage);
    }
    ...
    protected function _bulkRowAction($title, $url = null, $options = array(), $confirmMessage = false) {
        ...
        $output = $this->Html->link($this->Html->Tag('i','').$title, $url, $options);
        return $output;
    }
    ...
    public function adminAction($title, $url, $options = array()) {
        ...
        if (strcasecmp($options['method'], 'post') == 0) {
            return $this->Html->tag('li', $this->Form->postLink($title, $url, $options)
            );
        }
    ...
    public function adminTab($title, $url, $options = array()) {
        return $this->Html->tag('li', $this->Html->link($title, $url, Hash::merge(array(
                                    'data-toggle' => 'tab',
                                        ), $options)
                        )
        );
            ...
            foreach ($tabs as $title => $tab) {
                if (!isset($tab['options']['type']) || (isset($tab['options']['type']) && (in_array($this->_View->viewVars['typeAlias'], $tab['options']['type'])))) {
                    ...
                    if ($this->adminTabs) {
                        ...
                        $output .= $this->adminTab(__d('dna', $title), '#' . $domId);
                    }
                }
            }
        }
        ...
        foreach ($boxNames as $title => $box) {
            ...
            if (!$issetType || $typeInTypeAlias) {
                ...
                $output .= $this->_View->element(
                        $element, array(), array('plugin' => $plugin)
                );
