 ...
 * UpgradeTask
 *
 * @package  Dna.Dna.Console.Command.Task
 * @since    1.5
	...
	public function getOptionParser() {
		return parent::getOptionParser()
			->addSubCommand('acl', array(
				'help' => __d('dna', 'Upgrade ACL database for core controllers.'),
				'parser' => array(
					'description' => __d('dna',
		...
		'before upgrading.</warning>', $this->nl(2)
			...
			->addSubCommand('settings', array(
				'help' => __d('dna', 'Create settings.json from database'),
			))
			->addSubCommand('bootstraps', array(
				'help' => __d('dna', 'Update Hook.bootstrap settings'),
			))
			->addSubCommand('links', array(
				'help' => __d('dna', 'Update Links in database'),
			))
			->addSubCommand('first_migrations', array(
				'help' => __d('dna', 'Create first migration records'),
			))
			->addSubCommand('all', array(
				'help' => __d('dna', 'Run all upgrade tasks'),
			));
	}
 ...
 */
	public function settings($keys = array()) {
		...
		if (file_exists(APP . 'Config' . DS . 'settings.json')) {
			$this->err(__d('dna', '<warning>Config/settings.json already exist</warning>'));
		} else {
			...
			if ($version = file_get_contents(APP . 'VERSION.txt')) {
				$Setting->write('Dna.version', $version);
			}
			...
			$this->out(__d('dna', '<success>Config/settings.yml created based on `settings` table</success>'));
		}
	}
	...
	public function links() {
		...
		foreach ($links as $link) {
			...
			if (isset($this->_controllerMap[$url['controller']])) {
				...
				$this->out(__d('dna', 'Updating Link %s', $Link->id));
				$this->warn(__d('dna', '- %s', $link['Link']['link']));
				$this->success(__d('dna', '+ %s', $linkString), 2);
				$Link->saveField('link', $linkString, false);
				$count++;
			}
		}
		$this->out(__d('dna', 'Links updated: %d rows', $count));
	}
 ...
 */
	public function bootstraps() {
		...
		$this->out(__d('dna', 'Hook.bootstraps updated'));
	}
...
/**
 * Create default FirstMigration records for installations using dna_data.sql
 */
 ...
 */
	public function all() {
		foreach ($this->OptionParser->subcommands() as $command) {
			...
			$this->out(__d('dna', 'Upgrade "%s"', $name));
			$this->$name();
		}
	}
	...
	public function execute() {
		...
		$commands = array_keys($this->OptionParser->subcommands('dna'));
		$command = $this->args[0];
		if ($command[0] != '_' && in_array($command, $commands)) {
			return $this->{$command}();
		} else {
			$this->out(__d('dna', 'Command not recognized'));
		}
	}
