...
class DnaEventsTest extends DnaTestCase {
	...
	public function setUp() {
		parent::setUp();
		DnaPlugin::unload('Example');
		DnaPlugin::load('Shops');
 ...
 */
	public function tearDown() {
		parent::tearDown();
		DnaPlugin::unload('Shops');
	}

/**
 * Indirectly test DnaEventManager::detachPluginSubscribers()
 * triggerred by calling DnaPlugin::unload(null)
 */
	public function testDetachPluginSubscribers() {
		...
		$event = Dna::dispatchEvent($eventName, $this->Users);
 ...
 */
	public function testDispatchUsersEvents() {
		...
		foreach ($eventNames as $name) {
			$event = Dna::dispatchEvent($name, $this->Users);
 ...
 */
	public function testDispatchNodesEvents() {
		...
		foreach ($eventNames as $name) {
			$event = Dna::dispatchEvent($name, $this->Nodes);
			$this->assertTrue($event->result, sprintf('Event: %s', $name));
 ...
 */
	public function testDispatchHelperEvents() {
		...
		$View = new View();
		foreach ($eventNames as $name) {
			$event = Dna::dispatchEvent($name, $View);
			$this->assertTrue($event->result, sprintf('Event: %s', $name));
