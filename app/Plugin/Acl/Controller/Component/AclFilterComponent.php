 ...
 *
 * @category Component
 * @package  Dna.Acl.Controller.Component
 * @version  1.0
...
class AclFilterComponent extends Component {
    ...
    public function initialize(Controller $controller) {
        ...
        if ($this->_config('multiRole')) {
            Dna::hookAdminTab('Users/admin_add', 'Roles', 'Acl.admin/roles');
            Dna::hookAdminTab('Users/admin_edit', 'Roles', 'Acl.admin/roles');
        }
    }
    ...
    protected function _configure() {
        ...
        if ($this->_config('autoLoginDuration')) {
            if (!function_exists('mcrypt_encrypt')) {
                $notice = __d('dna', '"AutoLogin" (Remember Me) disabled since mcrypt_encrypt is not available');
    ...
    public function getPermissions($model, $id) {
        ...
        foreach ($permissions as $permission) {
            ...
            if ($acos == 4) {
                ...
                $this->log(sprintf(
                                'Incomplete path for aco_id = %s:', $permission['Permission']['id']
                ));
                $this->log($path);
            }
