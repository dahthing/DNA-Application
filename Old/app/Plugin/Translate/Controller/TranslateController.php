 ...
 * @package  Dna.Translate.Controller
 * @version  1.0
 ...
 */
	public function admin_index($id = null, $modelAlias = null) {
		if ($id == null || $modelAlias == null) {
			$this->Session->setFlash(__d('dna', 'Invalid ID.'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'plugin' => null,
		...
		if (!is_array($config)) {
			$this->Session->setFlash(__d('dna', 'Invalid model.'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'plugin' => $plugin,
				'controller' => Inflector::pluralize($modelAlias),
				'action' => 'index',
		...
		if (!isset($record[$modelAlias]['id'])) {
			$this->Session->setFlash(__d('dna', 'Invalid record.'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'plugin' => $plugin,
				'controller' => Inflector::pluralize($modelAlias),
				'action' => 'index',
		...
		$this->set('title_for_layout', sprintf(__d('dna', 'Translations: %s'), $record[$modelAlias][$model->displayField]));
 ...
 */
	public function admin_edit($id = null, $modelAlias = null) {
		...
		if (!isset($this->params['named']['locale'])) {
			$this->Session->setFlash(__d('dna', 'Invalid locale'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'plugin' => null,
				'controller' => Inflector::pluralize($modelAlias),
				'action' => 'index',
		...
		$this->set('title_for_layout', sprintf(__d('dna', 'Translate content: %s (%s)'), $language['Language']['title'], $language['Language']['native']));
		...
		if (!empty($this->request->data)) {
			if ($model->saveTranslation($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'Record has been translated'), 'default', array('class' => 'success'));
				$this->redirect(array(
					'action' => 'index',
					$id,
					$modelAlias,
				...
				$this->Session->setFlash(__d('dna', 'Record could not be translated. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
		if (empty($this->request->data)) {
 ...
 */
	public function admin_delete($id = null, $modelAlias = null, $locale = null) {
		if ($locale == null || $id == null) {
			$this->Session->setFlash(__d('dna', 'Invalid Locale or ID'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'plugin' => null,
				'controller' => Inflector::pluralize($modelAlias),
				'action' => 'index',
		...
		if ($runtimeModel->deleteAll(array(
			...
			$this->Session->setFlash(__d('dna', 'Translation for the locale deleted successfully.'), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'Translation for the locale could not be deleted.'), 'default', array('class' => 'error'));
		}
