 ...
 *
 * @category Controller
 * @package  Dna.Comments.Controller
 * @version  1.0
 ...
 */
	public function admin_index() {
		$this->set('title_for_layout', __d('dna', 'Comments'));
 ...
 */
	public function admin_edit($id = null) {
		$this->set('title_for_layout', __d('dna', 'Edit Comment'));

		if (!$id && empty($this->request->data)) {
			$this->Session->setFlash(__d('dna', 'Invalid Comment'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->request->data)) {
			if ($this->Comment->save($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'The Comment has been saved'), 'default', array('class' => 'success'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__d('dna', 'The Comment could not be saved. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
		if (empty($this->request->data)) {
 ...
 */
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('dna', 'Invalid id for Comment'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if ($this->Comment->delete($id)) {
			$this->Session->setFlash(__d('dna', 'Comment deleted'), 'default', array('class' => 'success'));
			$this->redirect(array('action' => 'index'));
		}
	}
 ...
 */
	public function admin_process() {
		...
		if ($action == 'delete' &&
			...
			$this->Session->setFlash(__d('dna', 'Comments deleted'), 'default', array('class' => 'success'));
		} elseif ($action == 'publish' &&
			$this->Comment->updateAll(array('Comment.status' => true), array('Comment.id' => $ids))) {
			$this->Session->setFlash(__d('dna', 'Comments published'), 'default', array('class' => 'success'));
		} elseif ($action == 'unpublish' &&
			$this->Comment->updateAll(array('Comment.status' => false), array('Comment.id' => $ids))) {
			$this->Session->setFlash(__d('dna', 'Comments unpublished'), 'default', array('class' => 'success'));
		} else {
 ...
 */
	public function add($nodeId = null, $parentId = null) {
		if (!$nodeId) {
			$this->Session->setFlash(__d('dna', 'Invalid Node'), 'default', array('class' => 'error'));
			$this->redirect('/');
		}
		...
		if (!is_null($parentId) && !$this->Comment->isValidLevel($parentId)) {
			$this->Session->setFlash(__d('dna', 'Maximum level reached. You cannot reply to that comment.'), 'default', array('class' => 'error'));
		...
		if (!$continue) {
			$this->Session->setFlash(__d('dna', 'Comments are not allowed.'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'controller' => 'nodes',
				'action' => 'view',
				'type' => $node['Node']['type'],
		...
		if (!empty($this->request->data) && $continue === true) {
			...
			if ($success) {
				if ($type['Type']['comment_approve']) {
					$messageFlash = __d('dna', 'Your comment has been added successfully.');
				} else {
					$messageFlash = __d('dna', 'Your comment will appear after moderation.');
				}
	...
	protected function _spam_protection($continue, $type, $node) {
		if (!empty($this->request->data) &&
			...
			if ($this->Akismet->isCommentSpam()) {
				$continue = false;
				$this->Session->setFlash(__d('dna', 'Sorry, the comment appears to be spam.'), 'default', array('class' => 'error'));
			}
		}
 ...
 */
	protected function _captcha($continue, $type, $node) {
		if (!empty($this->request->data) &&
			...
			$continue = false;
			$this->Session->setFlash(__d('dna', 'Invalid captcha entry'), 'default', array('class' => 'error'));
		}
	...
	protected function _sendEmail($node, $data) {
		...
		try {
			$email->from(Configure::read('Site.title') . ' ' .
				'<dna@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME'])) . '>')
				->to(Configure::read('Site.email'))
				->subject('[' . Configure::read('Site.title') . '] ' .
					__d('dna', 'New comment posted under') . ' ' . $node['Node']['title'])
