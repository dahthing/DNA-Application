Dna.Wysiwyg.Ckeditor = {
	...
	setup: function(el, config) {
		var preset = null;
		var defaults = {
			filebrowserBrowseUrl: Dna.Wysiwyg.attachmentsPath,
			filebrowserImageBrowseUrl: Dna.Wysiwyg.attachmentsPath
		};
		if (config.preset === false) {
			...
			preset = Dna.Wysiwyg.Ckeditor.presets[config.preset];
		}
...
Dna.Wysiwyg.choose = function(url, title, description) {
	...
	if (typeof paramsObj['CKEditorFuncNum'] != 'undefined') {
		window.top.opener.CKEDITOR.tools.callFunction(paramsObj['CKEditorFuncNum'], Dna.Wysiwyg.uploadsPath + url);
		window.top.close();
	}
}