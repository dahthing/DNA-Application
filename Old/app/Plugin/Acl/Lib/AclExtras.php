 ...
 */
	protected $_skipList = array(
		'controllers/DnaApp',
		'controllers/DnaError',
		'controllers/Dna/DnaApp',
		'controllers/Dna/DnaError',
	);
 ...
 */
	public function aco_update_contents($params = array()) {
		...
		if (!class_exists($model)) {
			$this->err(__d('dna', 'Model %s cannot be found.', $model));
			return false;
		}
		...
		foreach ($rows as $row) {
			...
			$this->out(__d('dna', 'Created Aco node: %s', $path), 1, Shell::VERBOSE);
		}
		return true;
	}
	...
	public function aco_update($params = array()) {
		...
		if (empty($params['plugin'])) {
			...
			if (!in_array($plugin, App::objects('plugin')) || !CakePlugin::loaded($plugin)) {
				$this->err(__d('dna', '<error>Plugin %s not found or not activated</error>', $plugin));
				return false;
			}
		...
		if ($this->_clean) {
			$this->out(__d('dna', '<success>Aco Sync Complete</success>'));
		} else {
			$this->out(__d('dna', '<success>Aco Update Complete</success>'));
		}
		return true;
	}
	...
	protected function _updateControllers($root, $controllers, $plugin = null) {
		...
		foreach ($controllers as $controller) {
			...
			if (in_array($path, $this->_skipList)) {
				$this->out(__d('dna', 'Skipped Aco node: <warning>%s</warning>', $path), 1, Shell::VERBOSE);
				continue;
			}
 ...
 */
	protected function _checkNode($path, $alias, $parentId = null) {
		...
		if (!$node) {
			...
			$this->created++;
			$this->out(__d('dna', 'Created Aco node: <success>%s</success>', $path), 1, Shell::VERBOSE);
		} else {
			$node = $node[0];
		}
		return $node;
			...
			foreach ($actionNodes as $action) {
				if (!isset($methodFlip[$action['Aco']['alias']])) {
					...
					if ($this->Aco->delete()) {
						...
						$this->out(__d('dna', 'Deleted Aco node: <warning>%s</warning>', $path), 1, Shell::VERBOSE);
					}
				}
			}
		}
 ...
 */
	public function verify() {
		...
		if ($return === true) {
			$this->out(__d('dna', '<success>Tree is valid and strong</success>'));
		} else {
 ...
 */
	public function recover() {
		...
		if ($return === true) {
			$this->out(__d('dna', 'Tree has been recovered, or tree did not need recovery.'));
		} else {
			$this->err(__d('dna', '<error>Tree recovery failed.</error>'));
			return false;
		}
	}
