 ...
 */
	public function upgrade() {
		...
		if (empty($root)) {
			return __d('dna', 'No root node found');
		} else {
			$root = $root[0];
		}
		...
		if ($upgraded) {
			return array(__d('dna', '<warning>ACL Database seems to have already been upgraded</warning>'));
		}
			...
			foreach ($controllers as $controllerName) {
				...
				if ($controllerNode) {
					...
					if (empty($correctControllerNode)) {
						$errors[] = __d('dna', '%s not found', $controllerPath);
					}
				}
			}
		}
