 ...
 *
 * @package     Dna.Acl.Console.Command
 ...
 **/
	...
	public function getOptionParser() {
		$plugin = array(
			'short' => 'p',
			'help' => __d('dna', 'Plugin to process'),
		);
		return parent::getOptionParser()
			->description(__d('dna', "Better manage, and easily synchronize you application's ACO tree"))
			->addSubcommand('aco_update', array(
				'parser' => array(
					'options' => compact('plugin'),
				),
				'help' => __d('dna', 'Add new ACOs for new controllers and actions. Does not remove nodes from the ACO table.')
			))
			->addSubcommand('aco_sync', array(
				'parser' => array(
					'options' => compact('plugin'),
				),
				'help' => __d('dna', 'Perform a full sync on the ACO table.' .
			...
			->addSubcommand('aco_sync_contents', array(
				'help' => __d('dna', 'Perform a full content sync on the ACO table.' .
				...
				'parser' => array(
					'arguments' => array(
						'model' => array(
							'required' => true,
							'help' => __d('dna', 'The content model name '),
						)
					),
				),
			))
			->addSubcommand('verify', array(
				'help' => __d('dna', 'Verify the tree structure of either your Aco or Aro Trees'),
				'parser' => array(
					'arguments' => array(
						'type' => array(
							'required' => true,
							'help' => __d('dna', 'The type of tree to verify'),
			...
			->addSubcommand('recover', array(
				'help' => __d('dna', 'Recover a corrupted Tree'),
				'parser' => array(
					'arguments' => array(
						'type' => array(
							'required' => true,
							'help' => __d('dna', 'The type of tree to recover'),
							'choices' => array('aco', 'aro')
						)
					)
				)
