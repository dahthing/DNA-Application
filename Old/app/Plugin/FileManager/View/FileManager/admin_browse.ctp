...
$this->Html
	...
	->addCrumb(__d('dna', 'File Manager'), '/' . $this->request->url);

?>

<?php $this->start('actions'); ?>
<?php
	echo $this->FileManager->adminAction(__d('dna', 'Upload here'),
		array('controller' => 'file_manager', 'action' => 'upload'),
		$path
	);
	echo $this->FileManager->adminAction(__d('dna', 'Create directory'),
		array('controller' => 'file_manager', 'action' => 'create_directory'),
		$path
	);
	echo $this->FileManager->adminAction(__d('dna', 'Create file'),
...
<div class="directory-content">
	...
	<?php
		...
		foreach ($content['0'] as $directory):
			...
			$actions[] = $this->FileManager->linkDirectory(__d('dna', 'Open'), $fullpath . DS);
			if ($this->FileManager->inPath($deletablePaths, $fullpath)) {
				$actions[] = $this->FileManager->link(__d('dna', 'Delete'), array(
			...
			$rows[] = array(
				$this->Html->image('/dna/img/icons/folder.png'),
		...
		foreach ($content['1'] as $file):
			...
			if ($icon == 'picture.png'):
				...
				$actions[] = $this->Html->link(__d('dna', 'View'),
					$image,
					$thickboxOptions
				);
			else:
				$linkFile = $this->FileManager->linkFile($file, $fullpath);
				$actions[] = $this->FileManager->link(__d('dna', 'Edit'),
					array(
						'plugin' => 'file_manager',
			...
			$actions = $this->Html->div('item-actions', implode(' ', $actions));
			$rows[] = array(
				$this->Html->image('/dna/img/icons/' . $icon),
				$linkFile,
				$actions,
			);
		endforeach;
