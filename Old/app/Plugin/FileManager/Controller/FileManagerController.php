 ...
 * @package  Dna.FileManager.Controller
 * @version  1.0
 ...
 */
	public function admin_browse() {
		...
		$this->set('title_for_layout', __d('dna', 'File Manager'));

		$path = realpath($path) . DS;
		$regex = '/^' . preg_quote(realpath(APP), '/') . '/';
		if (preg_match($regex, $path) == false) {
			$this->Session->setFlash(__d('dna', 'Path %s is restricted', $path), 'default', array('class' => 'error'));
			$path = APP;
		}
		...
		if (in_array(basename($path), $blacklist) || preg_match($regex, $path)) {
			$this->Session->setFlash(__d('dna', sprintf('Path %s is restricted', $path)), 'default', array('class' => 'error'));
			$path = dirname($path);
		}
 ...
 */
	public function admin_editfile() {
		...
		$this->set('title_for_layout', __d('dna', 'Edit file: %s', $path));
		...
		if (!empty($this->request->data) ) {
			if ($this->file->write($this->request->data['FileManager']['content'])) {
				$this->Session->setFlash(__d('dna', 'File saved successfully'), 'default', array('class' => 'success'));
			}
		}
 ...
 */
	public function admin_upload() {
		$this->set('title_for_layout', __d('dna', 'Upload'));
		...
		if (isset($this->request->data['FileManager']['file']['tmp_name']) &&
			...
			$this->Session->setFlash(__d('dna', 'File uploaded successfully.'), 'default', array('class' => 'success'));
 ...
 */
	public function admin_delete_file() {
		...
		if (file_exists($path) && unlink($path)) {
			$this->Session->setFlash(__d('dna', 'File deleted'), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'An error occured'), 'default', array('class' => 'error'));
		}
 ...
 */
	public function admin_delete_directory() {
		...
		if (is_dir($path) && rmdir($path)) {
			$this->Session->setFlash(__d('dna', 'Directory deleted'), 'default', array('class' => 'success'));
		} else {
			$this->Session->setFlash(__d('dna', 'An error occured'), 'default', array('class' => 'error'));
		}
 ...
 */
	public function admin_create_directory() {
		$this->set('title_for_layout', __d('dna', 'Create Directory'));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->folder->create($path . $this->request->data['FileManager']['name'])) {
				$this->Session->setFlash(__d('dna', 'Directory created successfully.'), 'default', array('class' => 'success'));
				$redirectUrl = Router::url(array('controller' => 'file_manager', 'action' => 'browse'), true) . '?path=' . urlencode($path);
				$this->redirect($redirectUrl);
			} else {
				$this->Session->setFlash(__d('dna', 'An error occured'), 'default', array('class' => 'error'));
 ...
 */
	public function admin_create_file() {
		$this->set('title_for_layout', __d('dna', 'Create File'));
		...
		if (!empty($this->request->data)) {
			if (touch($path . $this->request->data['FileManager']['name'])) {
				$this->Session->setFlash(__d('dna', 'File created successfully.'), 'default', array('class' => 'success'));
				$redirectUrl = Router::url(array('controller' => 'file_manager', 'action' => 'browse'), true) . '?path=' . urlencode($path);
				$this->redirect($redirectUrl);
			} else {
				$this->Session->setFlash(__d('dna', 'An error occured'), 'default', array('class' => 'error'));
