 ...
 */
	protected function _generateAssets() {
		$file = CakePlugin::path('Dna') . 'webroot' . DS . 'css' . DS . 'dna-bootstrap.css';
		if (!file_exists($file)) {
 ...
 */
	public function index() {
		$this->_check();
		$this->set('title_for_layout', __d('dna', 'Installation: Welcome'));
	}
 ...
 */
	public function database() {
		$this->_check();
		$this->set('title_for_layout', __d('dna', 'Step 1: Database'));
 ...
 */
	public function data() {
		$this->_check();
		$this->set('title_for_layout', __d('dna', 'Step 2: Build database'));
		...
		if (!empty($sources)) {
			$this->Session->setFlash(
				__d('dna', 'Warning: Database "%s" is not empty.', $ds->config['database']),
		...
		if (isset($this->params['named']['run'])) {
			...
			$InstallManager = new InstallManager();
			$result = $InstallManager->createDnaFile();
 ...
 */
	public function finish($token = null) {
		$this->set('title_for_layout', __d('dna', 'Installation successful'));
		$this->_check();
		...
		if ($result) {
			...
			$this->log(__d('dna', 'Installation failed: Unable to create settings file'));
		}
