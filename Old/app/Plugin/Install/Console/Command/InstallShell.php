 ...
 *
 * @category Shell
 * @package  Dna.Install.Console.Command
 * @version  1.0
 ...
 */
	public function getOptionParser() {
		...
		$parser->description(__d('dna', 'Install Utilities'))
			->addSubcommand('main', array(
				'help' => 'Generate dna.php, database.php, settings.json and create admin user.',
				'parser' => array(
					'description' => 'Generate dna.php, database.php, settings.json and create admin user.',
				)
			))
	...
	public function main() {
		...
		$install['Install']['datasource'] = $this->in(__d('dna', 'DataSource'), array(
			'Mysql',
			'Sqlite',
			'Postgres',
			'Sqlserver'
		...
		$install['Install']['host'] = $this->in(__d('dna', 'Host'), null, 'localhost');
		$install['Install']['login'] = $this->in(__d('dna', 'Login'), null, 'root');
		$install['Install']['password'] = $this->in(__d('dna', 'Password'), null, '');
		$install['Install']['database'] = $this->in(__d('dna', 'Database'), null, 'dna');
		$install['Install']['prefix'] = $this->in(__d('dna', 'Prefix'), null, '');
		$install['Install']['port'] = $this->in(__d('dna', 'Port'), null, null);
		...
		$InstallManager->createDnaFile();

		$this->out();
		$this->out('Create Admin user:');

		do {
			$username = $this->in(__d('dna', 'Username'), null, null);
			if (empty($username)) {
		...
		do {
			$password = $this->in(__d('dna', 'Password'));
			$verify = $this->in(__d('dna', 'Verify Password'));
		...
		$this->out();
		$this->success('Congratulations, Dna has been installed successfully.');
	}
 ...
 */
	...
	public function setup_acos() {
		...
		$Role->Behaviors->attach('Dna.Aliasable');
		...
		foreach ($setup as $aco => $roles) {
			foreach ($roles as $roleId) {
				...
				if ($Permission->allow($aro, $aco)) {
					$this->success(__d('dna', 'Permission %s granted to %s', $aco, $Role->byId($roleId)));
				}
			}
		}
	}
