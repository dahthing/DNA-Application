 ...
 */
	public function admin_index() {
		$this->set('title_for_layout', __d('dna', 'Contacts'));
 ...
 */
	public function admin_add() {
		$this->set('title_for_layout', __d('dna', 'Add Contact'));

		if (!empty($this->request->data)) {
			$this->Contact->create();
			if ($this->Contact->save($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'The Contact has been saved'), 'default', array('class' => 'success'));
				$this->Dna->redirect(array('action' => 'edit', $this->Contact->id));
			} else {
				$this->Session->setFlash(__d('dna', 'The Contact could not be saved. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
	}
 ...
 */
	public function admin_edit($id = null) {
		$this->set('title_for_layout', __d('dna', 'Edit Contact'));

		if (!$id && empty($this->request->data)) {
			$this->Session->setFlash(__d('dna', 'Invalid Contact'), 'default', array('class' => 'error'));
 ...
 */
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('dna', 'Invalid id for Contact'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if ($this->Contact->delete($id)) {
			$this->Session->setFlash(__d('dna', 'Contact deleted'), 'default', array('class' => 'success'));
			$this->redirect(array('action' => 'index'));
		}
	}
 ...
 */
	public function view($alias = null) {
		...
		if (!empty($this->request->data) && $continue === true) {
			...
			if ($continue === true) {
				//$this->Session->setFlash(__d('dna', 'Your message has been received.'));
				//unset($this->request->data['Message']);

				echo $this->flash(__d('dna', 'Your message has been received...'), '/');
			}
		}
	...
	protected function _spam_protection($continue, $contact) {
		if (!empty($this->request->data) &&
			...
			if ($this->Akismet->isCommentSpam()) {
				$continue = false;
				$this->Session->setFlash(__d('dna', 'Sorry, the message appears to be spam.'), 'default', array('class' => 'error'));
			}
		}
	...
	protected function _send_email($continue, $contact) {
		...
		try {
			$email->from($this->request->data['Message']['email'])
				...
				->subject(__d('dna', '[%s] %s', $siteTitle, $contact['Contact']['title']))
