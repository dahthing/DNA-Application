...
App::uses('DnaJson', 'Dna.Lib');

/**
 * DnaJsonReader
 *
 * @package  Dna.Dna.Lib.Configure
 * @since    1.5
 * @author   Fahad Ibnay Heylaal <contact@fahad19.com>
 * @license  http://www.opensource.org/licenses/mit-license.php The MIT License
 * @link     http://www.dna.org
 */
class DnaJsonReader implements ConfigReaderInterface {
 ...
 */
	public function read($key) {
		if (strpos($key, '..') !== false) {
			throw new ConfigureException(__d('dna', 'Cannot load configuration files with ../ in them.'));
		}
		...
		if (!is_file($file)) {
			if (!is_file(substr($file, 0, -4))) {
				throw new ConfigureException(__d('dna', 'Could not load configuration files: %s or %s', $file, substr($file, 0, -4)));
			}
		}
 ...
 */
	public function dump($filename, $data) {
		...
		$contents = DnaJson::stringify($data, $options);
