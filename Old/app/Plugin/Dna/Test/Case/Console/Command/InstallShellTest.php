...
App::uses('InstallShell', 'Dna.Console/Command');
 ...
 */
class InstallShellTest extends DnaTestCase {
 ...
 */
	public function setUp() {
		parent::setUp();
		App::build(array(
			'Plugin' => array(CakePlugin::path('Dna') . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS),
			'View' => array(CakePlugin::path('Dna') . 'Test' . DS . 'test_app' . DS . 'View' . DS),
		), App::PREPEND);
	}
 ...
 */
	public function tearDown() {
		...
		$files = $Folder->find('dna_.*');
		foreach ($files as $file) {
			unlink(TMP . $file);
		}
		$Folder = new Folder(CakePlugin::path('Dna') . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS . 'Example');
		$Folder->delete();
		$Folder = new Folder(CakePlugin::path('Dna') . 'Test' . DS . 'test_app' . DS . 'View' . DS . 'Themed' . DS . 'Minimal');
		$Folder->delete();
	}
 ...
 */
	public function testInstallPlugin() {
		...
		$Shell->args = array('plugin', 'shama', 'dna');
		$Shell->main();
	}
 ...
 */
	public function testComposerInstall() {
		...
		$prop = $Shell->getProperty('_DnaPlugin');
		$prop->setAccessible(true);
		$DnaPlugin = $this->getMock('DnaPlugin');
		$prop->setValue($ShellMock, $DnaPlugin);

		$DnaPlugin->expects($this->once())
			->method('getData')
			->will($this->returnValue(true));
