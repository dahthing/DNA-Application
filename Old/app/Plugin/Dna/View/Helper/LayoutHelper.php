 ...
 */
	public function __call($method, $params) {
		$mapMethods = array(
			...
			'adminRowActions' => array('Dna', 'adminRowActions'),
			'adminTabs' => array('Dna', 'adminTabs'),
			'adminMenus' => array('Dna', 'adminMenus'),
		);

		if (!isset($mapMethods[$method])) {
			trigger_error(__d('dna', 'Method %1$s::%2$s does not exist', get_class($this), $method), E_USER_WARNING);
			return;
		}
 ...
 *
 * Shows dna.js file along with useful information like the applications's basePath, etc.
 *
 * Also merges Configure::read('Js') with the Dna js variable.
 * So you can set javascript info anywhere like Configure::write('Js.my_var', 'my value'),
 * and you can access it like 'Dna.my_var' in your javascript.
 *
 * @return string
 */
	public function js() {
		...
		if (isset($this->params['locale'])) {
			$dna['basePath'] = Router::url('/' . $this->params['locale'] . '/');
		} else {
			$dna['basePath'] = Router::url('/');
		}
		$dna['params'] = array(
		...
		if (is_array(Configure::read('Js'))) {
			$dna = Hash::merge($dna, Configure::read('Js'));
		}
		return $this->Html->scriptBlock('var Dna = ' . $this->Js->object($dna) . ';');
	}
 ...
 */
	public function status($value) {
		...
		if (method_exists($this->Html, 'icon')) {
			...
			if (empty($this->_View->DnaHtml)) {
				$this->_View->Helpers->load('Dna.DnaHtml');
			}
			return $this->_View->DnaHtml->icon($icon, compact('class'));
		}
	}
 ...
 *
 * @deprecated Will be removed in the future. See DnaHelper::adminRowAction()
 */
 ...
 */
	public function filter($content) {
		Dna::dispatchEvent('Helper.Layout.beforeFilter', $this->_View, array('content' => &$content));
		$content = $this->filterElements($content);
		Dna::dispatchEvent('Helper.Layout.afterFilter', $this->_View, array('content' => &$content));
		return $content;
	}
