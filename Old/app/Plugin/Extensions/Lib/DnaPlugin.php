...
/**
 * DnaPlugin utility class
 *
 * PHP version 5
 *
 * @category Component
 * @package  Dna.Extensions.Lib
 * @version  1.4
 * @since    1.4
 ...
 */
class DnaPlugin extends Object {
		...
		foreach ($pluginPaths as $pluginPath) {
			...
			foreach ($pluginFolders[0] as $pluginFolder) {
				if (substr($pluginFolder, 0, 1) != '.') {
					if (!$this->_isDnaPlugin($pluginPath, $pluginFolder)) {
						continue;
					}
...
/**
 * Checks wether $pluginDir/$path is a Dna plugin
 *
 * @param string $pluginDir plugin directory
 * @param string $path plugin alias
 * @return bool true if path is a Dna plugin
 */
	protected function _isDnaPlugin($pluginDir, $path) {
 ...
 */
	public function activate($plugin) {
		if (CakePlugin::loaded($plugin)) {
			return __d('dna', 'Plugin "%s" is already active.', $plugin);
		}
		...
		if (!isset($pluginActivation) ||
			...
			if ($dependencies) {
				...
				return __d('dna', 'Plugin "%s" depends on "%s" plugin.', $plugin, $missingPlugin);
			}
			return __d('dna', 'Plugin "%s" could not be activated. Please, try again.', $plugin);
		}
	}
 ...
 */
	public function deactivate($plugin) {
		if (!CakePlugin::loaded($plugin)) {
			return __d('dna', 'Plugin "%s" is not active.', $plugin);
		}
		$pluginActivation = $this->getActivator($plugin);
		if (!isset($pluginActivation) ||
			...
			DnaPlugin::unload($plugin);
			Cache::delete('EventHandlers', 'cached_settings');
			return true;
		} else {
			return __d('dna', 'Plugin could not be deactivated. Please, try again.');
		}
	}
 ...
 */
	public static function unload($plugin) {
		$eventManager = DnaEventManager::instance();
		if ($eventManager instanceof DnaEventManager) {
			if ($plugin == null) {
 ...
 */
	public function delete($plugin) {
		if (empty($plugin)) {
			throw new InvalidArgumentException(__d('dna', 'Invalid plugin'));
		}
	...
	protected function _move($dir, $plugin, $bootstraps) {
		...
		if ($dir === 'up') {
			...
			if (in_array($plugin, $dependencies)) {
				return __d('dna', 'Plugin %s depends on %s', $swap, $plugin);
			}
			$reordered = array_merge($before, (array)$swap, (array)$plugin);
		} else {
			return __d('dna', 'Invalid direction');
		}
		$reordered = array_merge($reordered, $after);

		if ($this->_isBuiltin($swap)) {
			return __d('dna', 'Plugin %s cannot be reordered', $swap);
		}
