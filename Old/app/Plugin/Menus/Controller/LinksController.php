 ...
 */
	public function admin_toggle($id = null, $status = null) {
		$this->Dna->fieldToggle($this->Link, $id, $status);
	}
 ...
 */
	public function admin_index() {
		...
		$this->set('title_for_layout', __d('dna', 'Links: %s', $menu['Menu']['title']));
 ...
 */
	public function admin_add($menuId = null) {
		$this->set('title_for_layout', __d('dna', 'Add Link'));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->Link->save($this->request->data)) {
				$this->Session->setFlash(__d('dna', 'The Link has been saved'), 'default', array('class' => 'success'));
				...
				$this->Session->setFlash(__d('dna', 'The Link could not be saved. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
 ...
 */
	public function admin_edit($id = null) {
		$this->set('title_for_layout', __d('dna', 'Edit Link'));

		if (!$id && empty($this->request->data)) {
			$this->Session->setFlash(__d('dna', 'Invalid Link'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'controller' => 'menus',
				'action' => 'index',
			));
 ...
 */
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('dna', 'Invalid id for Link'), 'default', array('class' => 'error'));
			$this->redirect(array(
				'controller' => 'menus',
				'action' => 'index',
			));
		...
		if ($this->Link->delete($id)) {
			$this->Session->setFlash(__d('dna', 'Link deleted'), 'default', array('class' => 'success'));
			$this->redirect(array(
				'action' => 'index',
				'?' => array(
 ...
 */
	public function admin_process($menuId = null) {
		...
		if ($action == 'delete' &&
			...
			$this->Session->setFlash(__d('dna', 'Links deleted.'), 'default', array('class' => 'success'));
		} elseif ($action == 'publish' &&
			$this->Link->updateAll(array('Link.status' => 1), array('Link.id' => $ids))) {
			$this->Session->setFlash(__d('dna', 'Links published'), 'default', array('class' => 'success'));
		} elseif ($action == 'unpublish' &&
			$this->Link->updateAll(array('Link.status' => 0), array('Link.id' => $ids))) {
			$this->Session->setFlash(__d('dna', 'Links unpublished'), 'default', array('class' => 'success'));
		} else {
