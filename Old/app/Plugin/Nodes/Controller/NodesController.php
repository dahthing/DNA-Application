 ...
 */
	public function admin_toggle($id = null, $status = null) {
		$this->Dna->fieldToggle($this->Node, $id, $status);
	}
 ...
 */
	public function admin_index() {
		$this->set('title_for_layout', __d('dna', 'Content'));
 ...
 */
	public function admin_create() {
		$this->set('title_for_layout', __d('dna', 'Create content'));
 ...
 */
	public function admin_add($typeAlias = 'node') {
		...
		if (!isset($type['Type']['alias'])) {
			$this->Session->setFlash(__d('dna', 'Content type does not exist.'));
		...
		if (!empty($this->request->data)) {
			...
			if ($this->Node->saveNode($this->request->data, $typeAlias)) {
				Dna::dispatchEvent('Controller.Nodes.afterAdd', $this, array('data' => $this->request->data));
				$this->Session->setFlash(__d('dna', '%s has been saved', $type['Type']['title']), 'default', array('class' => 'success'));
				$this->Dna->redirect(array('action' => 'edit', $this->Node->id));
			} else {
				$this->Session->setFlash(__d('dna', '%s could not be saved. Please, try again.', $type['Type']['title']), 'default', array('class' => 'error'));
			}
		} else {
			$this->request->data['Node']['user_id'] = $this->Session->read('Auth.User.id');

			$this->set('title_for_layout', __d('dna', 'Create content: %s', $type['Type']['title']));
 ...
 */
	public function admin_edit($id = null) {
		if (!$id && empty($this->request->data)) {
			$this->Session->setFlash(__d('dna', 'Invalid content'), 'default', array('class' => 'error'));
		...
		if (!empty($this->request->data)) {
			if ($this->Node->saveNode($this->request->data, $typeAlias)) {
				Dna::dispatchEvent('Controller.Nodes.afterEdit', $this, compact('data'));
				$this->Session->setFlash(__d('dna', '%s has been saved', $type['Type']['title']), 'default', array('class' => 'success'));
				$this->Dna->redirect(array('action' => 'edit', $this->Node->id));
			} else {
				$this->Session->setFlash(__d('dna', '%s could not be saved. Please, try again.', $type['Type']['title']), 'default', array('class' => 'error'));
		...
		$this->set('title_for_layout', __d('dna', 'Edit %s: %s', $type['Type']['title'], $this->request->data['Node']['title']));
		$this->_setCommonVariables($type);
	}
 ...
 */
	public function admin_update_paths() {
		if ($this->Node->updateAllNodesPaths()) {
			$messageFlash = __d('dna', 'Paths updated.');
			$class = 'success';
		} else {
			$messageFlash = __d('dna', 'Something went wrong while updating paths.' . "\n" . 'Please try again');
			$class = 'error';
		}
 ...
 */
	public function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__d('dna', 'Invalid id for Node'), 'default', array('class' => 'error'));
			$this->redirect(array('action' => 'index'));
		}
		if ($this->Node->delete($id)) {
			$this->Session->setFlash(__d('dna', 'Node deleted'), 'default', array('class' => 'success'));
			$this->redirect(array('action' => 'index'));
		}
	}
 ...
 */
	public function admin_process() {
		...
		if ($actionProcessed) {
			switch ($action) {
				case 'delete':
					$messageFlash = __d('dna', 'Nodes deleted');
				break;
				case 'publish':
					$messageFlash = __d('dna', 'Nodes published');
				break;
				case 'unpublish':
					$messageFlash = __d('dna', 'Nodes unpublished');
				break;
				case 'promote':
					$messageFlash = __d('dna', 'Nodes promoted');
				break;
				case 'unpromote':
					$messageFlash = __d('dna', 'Nodes unpromoted');
				break;
			}
			$this->Session->setFlash($messageFlash, 'default', array('class' => 'success'));
			Dna::dispatchEvent($eventName, $this, compact($ids));
		} else {
	...
	public function index() {
		...
		$this->paginate['Node']['conditions'] = array(
			...
			'OR' => array(
				...
				'Node.visibility_roles LIKE' => '%"' . $this->Dna->roleId . '"%',
			),
		);
		...
		if (isset($this->request->params['named']['type'])) {
			...
			if (!isset($type['Type']['id'])) {
				$this->Session->setFlash(__d('dna', 'Invalid content type.'), 'default', array('class' => 'error'));
				$this->redirect('/');
			}
		...
		if ($this->usePaginationCache) {
			$cacheNamePrefix = 'nodes_index_' . $this->Dna->roleId . '_' . Configure::read('Config.language');
			if (isset($type)) {
 ...
 */
	public function term() {
		...
		if (!isset($term['Term']['id'])) {
			$this->Session->setFlash(__d('dna', 'Invalid Term.'), 'default', array('class' => 'error'));
			$this->redirect('/');
		}
		...
		if ($this->usePaginationCache) {
			$cacheNamePrefix = 'nodes_term_' . $this->Dna->roleId . '_' . $this->request->params['named']['slug'] . '_' . Configure::read('Config.language');
			if (isset($type)) {
				$cacheNamePrefix .= '_' . $type['Type']['alias'];
			}
			$this->paginate['page'] = isset($this->request->params['named']['page']) ? $this->params['named']['page'] : 1;
 ...
 */
	public function promoted() {
		$this->set('title_for_layout', __d('dna', 'Nodes'));
		...
		if ($this->usePaginationCache) {
			...
			$cacheNamePrefix = 'nodes_promoted_' . $this->Dna->roleId . '_' . Configure::read('Config.language');
			if (isset($type)) {
				$cacheNamePrefix .= '_' . $type['Type']['alias'];
			}
			$this->paginate['page'] = isset($this->request->params['named']['page']) ? $this->params['named']['page'] : 1;
 ...
 */
	public function search($typeAlias = null) {
		...
		$nodes = $this->paginate('Node');
		$this->set('title_for_layout', __d('dna', 'Search Results: %s', $q));
	...
	public function view($id = null) {
		if (isset($this->request->params['named']['slug']) && isset($this->params['named']['type'])) {
			...
			$node = $this->Node->find('first', array(
				...
				'cache' => array(
					'name' => 'node_' . $this->Dna->roleId . '_' . $this->request->params['named']['type'] . '_' . $this->params['named']['slug'],
					'config' => 'nodes_view',
				),
			));
		} elseif ($id == null) {
			...
			$node = $this->Node->find('first', array(
				...
				'cache' => array(
					'name' => 'node_' . $this->Dna->roleId . '_' . $id,
					'config' => 'nodes_view',
				),
			));
